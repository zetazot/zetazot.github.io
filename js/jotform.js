...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    checkCalculation: function (calc) {
        ...
        try {
            if (![
                ...
                'file', 'other', 'mixed', 'sofort', 'payoneer', 'paysafe', 'gocardless'
    ...
    countTotal: function (prices) {
        ...
        $H(prices).each(function (pair) {
          ...
          if($(pair.key).checked === true && window.paymentType === 'subscription' && Array.from(document.querySelectorAll('.jfCard')).filter(function(el) {return el.dataset.type === 'control_stripe'}).length > 0){
  ...
  /**
   *  Checks if coupon code associated with selected prodcut on the front-end
   */
    ...
    checkCouponAppliedProducts: function (){
      ...
      if(window.discounted_product === selected_product_id || window.discounted_product === "all" ) {
   ...
   */
    ...
    applyCoupon: function (discount) {
        ...
        if(discount.products && discount.products[0]) {
          window.discounted_product = discount.products[0];
        }
